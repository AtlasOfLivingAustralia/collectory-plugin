 ...
 * @monkeyPatch MEW
 *
 * Patched to record entity uid where available.
 * Uses the existing (but unpopulated) uri field (more of a gorilla patch).
 *
 * 2001-06-22 applied patch to version 0.5.4
 *
 */
...
public class AuditLogListener implements PreDeleteEventListener, PostInsertEventListener, PostUpdateEventListener, Initializable {
  ...
  /* MEW change */
  def getEntityUid(event) {
      if (event && event.entity) {
        def entity = event.getEntity()
        if (entity.metaClass.hasProperty(entity, 'uid') && entity.'uid') {
          return entity.uid
        }
        if (entity.metaClass.hasProperty(entity, 'entityUid') && entity.'entityUid') {
  ...
  public boolean onPreDelete(final PreDeleteEvent event) {
    try {
      ...
      if (audit && !callHandlersOnly(event.getEntity())) {
        ...
        logChanges(null, map, null, entityId, 'DELETE', entityName, event)   /* MEW change */
      }
      if (audit) {
  ...
  public void onPostInsert(final PostInsertEvent event) {
    try {
      ...
      if (audit && !callHandlersOnly(event.getEntity())) {
        ...
        def entityName = entity.getClass().getName()
        def entityId = getEntityId(event)
        logChanges(map, null, null, entityId, 'INSERT', entityName, event)   /* MEW change */
      }
      if (audit) {
  ...
  private void onChange(final PostUpdateEvent event) {
    ...
    if (!callHandlersOnly(event.getEntity())) {
      logChanges(newMap, oldMap, event, entityId, 'UPDATE', entityName, event)   /* MEW change */
    }
    /**
   ...
   */
  def logChanges(newMap, oldMap, parentObject, persistedObjectId, eventName, className, event) {   /* MEW change */
      ...
      newMap.each({key, val ->
        if (val != oldMap[key]) {
          audit = new AuditLogEvent(
                  actor: this.getActor(),
                  uri: getEntityUid(event),   /* MEW change */
                  className: className,
                  eventName: eventName,
